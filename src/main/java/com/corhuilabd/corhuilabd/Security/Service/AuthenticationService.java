...
import com.corhuilabd.corhuilabd.Security.Role;
...
public class AuthenticationService {
    ...
    public User authenticate(LoginUserDto input, HttpServletRequest request, HttpServletResponse response) {
        // Permitir login tanto por email como por username
        Optional<User> userOpt = userRepository.findByEmail(input.getEmail());
        if (userOpt.isEmpty() && input.getEmail() != null) {
            userOpt = userRepository.findByUsername(input.getEmail());
        }
        ...
        // Establecer la autenticación en el contexto de seguridad y en la sesión HTTP
        SecurityContextHolder.getContext().setAuthentication(authentication);
        
        // Crear y establecer la sesión HTTP
    ...
    public void verifyUser(VerifyUserDto input) {
        ...
        if (optionalUser.isPresent()) {
            ...
            if (user.getVerificationCodeExpiresAt() == null) {
                throw new RuntimeException("El usuario ya está verificado o el código no existe.");
            }
